 ...
 * (std::vector of maps) for a keyword. If the return value is a collection, the
 * collection is returned. If it is only a single value, a std::vector
 * with length 1 is returned. If the keyword wasn't found, a std::vector with
